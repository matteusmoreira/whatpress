-- Enhanced Quota System Migration V2
-- Sistema de quotas avançado com alertas automáticos

-- Primeiro, limpar funções existentes que podem ter conflitos
DROP FUNCTION IF EXISTS public.calculate_quota_percentage CASCADE;
DROP FUNCTION IF EXISTS public.should_send_quota_alert CASCADE;
DROP FUNCTION IF EXISTS public.update_tenant_quota_counters CASCADE;
DROP FUNCTION IF EXISTS public.check_and_create_quota_alerts CASCADE;
DROP FUNCTION IF EXISTS public.trigger_update_quota_counters CASCADE;
...
CREATE FUNCTION public.calculate_quota_percentage(current_val INTEGER, max_val INTEGER)
...
CREATE FUNCTION public.should_send_quota_alert(
  tenant_uuid UUID,
  resource_type TEXT,
  alert_type TEXT
) RETURNS BOOLEAN AS $$
...
CREATE FUNCTION public.update_tenant_quota_counters(tenant_uuid UUID)
RETURNS VOID AS $$
DECLARE
  users_count INTEGER;
...
CREATE FUNCTION public.check_and_create_quota_alerts(tenant_uuid UUID)
RETURNS VOID AS $$
DECLARE
  quota_record RECORD;
  alert_message TEXT;
...
CREATE FUNCTION public.trigger_update_quota_counters()
RETURNS TRIGGER AS $$
BEGIN
